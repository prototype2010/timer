...
export function formChartBars(tasksGroupedArray) {
    ...
    return tasksGroupedArray.map(([hour, tasksArray = []]) => ({
        name: moment().set('hour', hour).minutes(0).second(0).format('HH:mm'),
        ...tasksArray.reduce((cumulative, {id, timeSpent}) => ({
            ...cumulative,
            [id]: timeSpent
        }), {})
    }))
...
function coerceTasksToMinutes(tasksObject) {
    ...
    return Object.entries(tasksObject)
        .map(([index, tasksArray]) => ([
            index,
...
function coerceTaskTimeToCurrentHour(time, currentHour) {
    ...
    if (taskTime.get('hour') === currentHour) {
        ...
        return taskTime.set('hour', currentHour).minutes(0).second(0);
    } else {
        return taskTime.set('hour', currentHour).add(1, 'hours').minutes(0).second(0);
    }
}

function convertTaskToMinutesRange({id, startTime, endTime, taskName}, currentHour) {
